...
{
	asm volatile(".macro load_matrixA_once  s_offset \n \
			s_load_dwordx8   s[s_matrixA_m16  + \\s_offset : s_matrixA_m16  + \\s_offset+7], s[s_matrixA_base:s_matrixA_base+1],     s[s_matrixA_sgpr_addr]    \n \
			s_add_u32        s[s_matrixA_sgpr_addr], 	s[s_matrixA_sgpr_addr], s[s_lda]										 \n \
			s_load_dwordx8   s[s_matrixA_m16  + \\s_offset+8 : s_matrixA_m16  + \\s_offset+15], s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]   \n \
			s_add_u32        s[s_matrixA_sgpr_addr], 	s[s_matrixA_sgpr_addr], s[s_lda]										 \n \
			s_load_dwordx8  s[s_matrixA_m16  + \\s_offset+16 : s_matrixA_m16  + \\s_offset+23], s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]   \n \
			s_add_u32        s[s_matrixA_sgpr_addr], 	s[s_matrixA_sgpr_addr], s[s_lda]										 \n \
			s_load_dwordx8  s[s_matrixA_m16  + \\s_offset+24 : s_matrixA_m16  + \\s_offset+31], s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]	 \n \
			s_add_u32        s[s_matrixA_sgpr_addr], 	s[s_matrixA_sgpr_addr], s[s_lda]										 \n \
		.endmacro \n"::);
	
	asm volatile("load_matrixA_once 0  \n");
}
...
__device__ void DotProductMacros()
{
	...
	//Double buffer to Load Matrix A from Constant Buffer;
	asm volatile(".macro DotProductGroupA groupindex  \n  \
					s_waitcnt lgkmcnt(0)  \n \
					load_matrixA_once  32 \n \
	...
	asm volatile(".macro DotProductGroupB groupindex \n \
					...
					/* switch to next 8xK elements of Matrix A: FP32 * 8 = 32*/    \n\
					.if  7 == \\groupindex												\n \
						s_add_u32  s[s_matrixA_koffset], s[s_matrixA_koffset], 8*4		\n \
						s_mov_b32		   s[s_matrixA_sgpr_addr], s[s_matrixA_koffset]	\n \
					.endif								\n \
	...
	int matrixCTranspose)
{
	...
	MajorLoopBegin();
	{
		...
		//Remove to have do double buffer
