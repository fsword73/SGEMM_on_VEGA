...
hipError_t Sgemm(hipFunction_t& kernel, float* A, float* B, float* C, int M, int N, int K, int lda, int ldb, int ldc, float* alpha, float* beta, int isTranspose, int m_tile_size, int n_tile_size, int req_work_group_size=128)
{
	char argbuf[68];
	size_t argsize = 0;
	...
	*((int*)& argbuf[argsize]) = 0; argsize += sizeof(int);
	...
	return hipModuleLaunchKernel(kernel, gdx, gdy, 1, req_work_group_size, 1, 1, 0, 0, NULL, &config[0]);
...
void test(int m, int n, int k, int m_tile_size, int n_tile_size, int printout, int iterations=10, int padding = 16, int req_work_group_size=128)
{
	...
	cout << "address " << deviceA << "," << deviceB << "," << deviceC << "\n";
	{
			...
			Sgemm(sgemmkernel, deviceA, deviceB, deviceC, test_m, test_n, test_k, lda, ldb, ldc, &alpha, &beta, 1, m_tile_size, n_tile_size, req_work_group_size);
...
int main(int argc, char** argv)
{
	...
	m_tile = 64;
	n_tile = 128;
	int req_work_group_size = 128;

	test(m, n, k, m_tile, n_tile, printout,  iterations, 16, req_work_group_size);
