/*
please note that the macro of optmiztion technology is not in official document.
...
__device__ void InitMatrixABInfo(int macro_tile_n)
{
	if (!(macro_tile_n == 256 || macro_tile_n == 128 || macro_tile_n == 64))
		return;
	...
	case 256:
		...
		break;
	case 128:
		asm volatile("s_lshl_b32	s[s_matixB_tile_offset], s[hipBlockIdx_x], 7\n");  //*Macro tile 64x128
		break;
	case 64:
		asm volatile("s_lshl_b32	s[s_matixB_tile_offset], s[hipBlockIdx_x], 6\n");  //*Macro tile 64x64
		break;
	}
...
__device__ void MajorLoopEnd() {
	...
	//asm volatile("label_major_loop_end:\n");
...
__device__ void InitMatrixCInfo(int macro_tile_n)
{
	if (!(macro_tile_n == 64 || macro_tile_n == 128 || macro_tile_n == 256))
		return;
...
//	asm volatile("s_add_u32			    s[s_matrixC_m_tile_offset],     s[s_matrixC_m_tile_offset], s[s_matrixC_m_tile_offset+1]\n");
	...
	case 256:
		...
		break;
	case 128:
		asm volatile("s_lshl_b32      s[s_matrixC_n_tile_offset],   s[hipBlockIdx_x] , 7\n");  //64 * 128 macro tile size
		break;
	case 64:
		asm volatile("s_lshl_b32      s[s_matrixC_n_tile_offset],   s[hipBlockIdx_x] , 6\n");  //64 * 128 macro tile size
		break;
	}
